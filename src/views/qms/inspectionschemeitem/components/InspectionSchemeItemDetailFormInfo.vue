  ...
  >
    <el-table :data="formData" class="-mt-10px">
      ...
      <el-table-column label="排序" min-width="150">
        <template #default="{ row, $index }">
          <el-form-item :prop="`${$index}.sortNumber`" :rules="formRules.sortNumber" class="mb-0px!">
            <el-input-number v-model="row.sortNumber" placeholder="请输入数字" />
          </el-form-item>
        </template>
      </el-table-column>
...
<script setup lang="ts">
  ...
  defineOptions({name: 'InspectionSchemeItemDetailFormInfo'})
  ...
  const formRules = reactive({
    inspectionItemConfigId: [{ required: true, message: '检测项配置表不能为空', trigger: 'blur' }],
    inspectionSchemeItemId: [{ required: true, message: '检测项目不能为空', trigger: 'blur' }],
    sortNumber: [{ required: true, message: '排序不能为空', trigger: 'blur' }],
  })
  const formRef = ref() // 表单 Ref

  /** 监听主表的关联字段的变化，加载对应的子表数据 */
  watch(
    () => props.inspectionSchemeItemId,
    async (val) => {
      // 1. 重置表单
      formData.value = []
      // 2. val 非空，则加载数据
      ...
      try {
        ...
        formData.value = await InspectionSchemeItemApi.getInspectionSchemeItemDetailListByInspectionSchemeItemId(val)
      } finally {
  ...
  const handleAdd = () => {
    ...
    row.inspectionSchemeItemId = props.inspectionSchemeItemId
