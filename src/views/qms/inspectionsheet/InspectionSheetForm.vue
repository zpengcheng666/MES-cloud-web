<template>
  <Dialog :title="dialogTitle" v-model="dialogVisible" >
    ...
    >
      <el-row>
        <el-col :span="12">
          <el-form-item label="物料类型" prop="materialConfigId">
            <el-input v-model="formData.materialName" placeholder="请选择物料类型" @click="openForm()" readonly />
          </el-form-item>
        </el-col>
        <el-col :span="12">
      ...
      <el-row>
        <el-col :span="12" v-if="formData.schemeType=='2' || formData.schemeType=='3'">
            ...
            <el-input v-model="formData.technologyName" placeholder="请选择工艺" @click="openProcessForm('create')" readonly>
              <template #suffix>
                <Icon
                  ...
                  @click="formData.needFirstInspection=undefined; formData.technologyName=undefined; formData.technologyId=undefined; formData.processId=undefined; procedureValue=[];formData.schemeId=undefined;inspectionSchemeList=[];"
                />
              </template>
            </el-input>
          </el-form-item>
        ...
        <el-col :span="12" v-if="formData.schemeType=='2'">
          <el-form-item label="是否首检" prop="needFirstInspection" >
            <el-select
                v-model="formData.needFirstInspection"
                placeholder="请选择是否首检"
                clearable
            >
              <el-option
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await InspectionSheetApi.getInspectionSheet(id)
    } finally {
...
const submitForm = async () => {
  ...
  try {
    ...
    if (formType.value === 'create') {
      await InspectionSheetApi.createInspectionSheet(data)
      message.success(t('common.createSuccess'))
    } else {
      await InspectionSheetApi.updateInspectionSheet(data)
 ...
 * 获取物料类型
 * */
const materialConfigData = ref({}) // 合同 Ref

const getMaterialConfig = (config) => {
  materialConfigData.value = config
  // 选中物料类型
  formData.value.materialConfigId = config.id
  formData.value.materialName = config.materialName
  // 修改物料类型 重新获取检验方案
...
const getInspectionSchemeList =async()=> {
  ...
  if(formData.value.schemeType == 2 && !formData.value.processId){
    inspectionSchemeList.value = []
    return
  }
