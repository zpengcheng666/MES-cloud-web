  ...
  <Dialog :title="dialogTitle" v-model="dialogVisible" width="1280" >
    ...
    <ContentWrap>
      <el-table v-loading="formLoading"
        ...
        <el-table-column label="当前类别" align="center" prop="materialTypeId" />
        <el-table-column label="主类别" align="center" prop="materialParentTypeId" />
        ...
        <el-table-column label="来源物料" align="center" prop="materialSourceId" />
...
/** 规则 列表 */
defineOptions({ name: 'MaterialConfigChoose' })
...
const emit = defineEmits(['getSource']) // 定义 success 事件，用于操作成功后的回调
/** 确认 **/
const submitForm = async () => {
  ...
  try {
    ...
    // 发送操作成功的事件
      debugger
    emit('getSource', selectionContract.value)
  } finally {
    formLoading.value = false
  }
}
 ...
 * 实现单选行
 */
const selectionContract = ref<EncodingRuleVO[]>()
