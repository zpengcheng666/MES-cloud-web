...
/** 合同发票 表单 */
defineOptions({ name: 'DMBpmContractInvoiceCreate' })
...
const submitForm = async () => {
  ...
  try {
    ...
    data.processKey = 'dm-contract-invoice-audit'
    await ContractInvoiceApi.createAndSubmitContractInvoice(data)
    message.success('发起成功')
    // 关闭当前 Tab
    delView(unref(currentRoute))
    await push({ name: 'ContractInvoice' })
  } finally {
...
onMounted(async () => {
  // 获取流程图
  const processDefinitionDetail = await DefinitionApi.getProcessDefinition(undefined,"dm-contract-invoice-audit")
