      ...
      <ContentWrap>
        <el-tabs v-model="subTabsName2">
          <el-tab-pane label="付款详情" name="contractPayment">
            <DMContractPaymentSchemeForm ref="contractPaymentFormRef" :contract-id="formData.contractId" :details="formData.paymentDetails" :schemeList="schemeData" :form-type="formType" />
          </el-tab-pane>
        </el-tabs>
      </ContentWrap>
...
/** 合同付款 表单 */
defineOptions({ name: 'DMContractPaymentCreate' })
...
const processDefineKey = 'dm-contract-payment-audit' // 流程定义 Key
/** 计算付款总金额 */
watch(
...
const submitForm = async () => {
  ...
  try {
    ...
    data.processKey = processDefineKey;
