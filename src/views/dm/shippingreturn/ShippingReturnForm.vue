...
<script setup lang="ts">
  ...
  /** 销售退货单 表单 */
  defineOptions({name: 'ShippingReturnForm'})
  ...
  const formRules = reactive({
    ...
    projectId: [{required: true, message: '合同不能为空', trigger: 'blur'}],
    ...
    if (id) {
      ...
      try {
        ...
        returnType.value=formData._value.returnType;
          detailInfo.value =await ShippingApi.getShippingDetailByProject(formData._value.projectId,formData._value.contractId ==undefined ? '0':formData._value.contractId,1 );
      } finally {
