  ...
  <Dialog v-model="dialogVisible" :title="dialogTitle">
    <ContentWrap>
      <el-form ref="queryFormRef" :inline="true" :model="queryParams" class="-mb-15px" label-width="68px">
        <el-form-item label="工装编号" prop="materialNumber">
          <el-input v-model="queryParams.materialNumber" class="!w-240px" clearable placeholder="请输入工装编号" />
        </el-form-item>
        <el-form-item label="工装名称" prop="materialName">
          <el-input v-model="queryParams.materialName" class="!w-240px" clearable placeholder="请输入工装名称" />
        </el-form-item>
        <el-form-item>
    ...
    <ContentWrap>
      <el-table ref="multipleTable" :data="materialDataList" style="height:400px;overflow-y: auto"
        ...
        <el-table-column align="center" label="工装编号" prop="materialNumber" />
    ...
    <template #footer>
      <el-button type="primary" @click="saveSelectedMaterial(selectionList.map((item) => item.id))"
...
import { ProcessPlanApi, MaterialVO } from "@/api/pdm/processPlan";
...
/** 打开弹窗 */
const open = async (processVersionId: string, procedureId: string, materialIdss: string[]) => {
  dialogVisible.value = true
  dialogTitle.value = '选择工装'
  selectedMaterialParams.processVersionId = processVersionId
  selectedMaterialParams.procedureId = procedureId
...
const saveSelectedMaterial = async (ids: number[]) => {
  try {
    ...
    await ProcessPlanDetailApi.saveSelectedResourceProcedure(selectedMaterialParams)
