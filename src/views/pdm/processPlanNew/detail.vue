              ...
              <el-row>
                ...
                <el-col :span="12">
                  <el-form-item label="工艺规程名称" prop="processName">
                    <el-input v-model="formProcessPlan.processName" disabled />
                  </el-form-item>
                </el-col>
              </el-row>
              <el-row>
...
/** 获得数据 */
const getInfo = async () => {
  detailData.value = await ProcessPlanApi.getPartDetailByTaskIdNew(props.id || queryId)
...
const partChange = async (val) => {
  ...
  queryParamsProcess.partVersionId = queryParams.partVersionId
...
const handleNodeClick = async (row) => {
  if (row.type === 1) {//零件节点
    ...
    //获取零件信息详情
    const val = await ProcessPlanApi.getPartDetailByTaskIdNew(row.taskId)
