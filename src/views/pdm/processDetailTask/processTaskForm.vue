...
const procedureIdArr = ref([]) //工序id数组
const processVersionIdArr = ref([]) // 版本id数组
...
/** 打开弹窗 */
const open = async (type: string, taskId?: string, partVersionIds?: string[], projectCodeStr?: string, id?: string[], processVersionId?: string) => {
  ...
  if (taskId) {
    ...
    try {
      formData.value = await ProcessDetailTaskApi.getProcessDetailTask(taskId);
    } finally {
  ...
  if(id){
    procedureIdArr.value = id;
  }
   if (projectCodeStr) {
     formData.value.projectCode = projectCodeStr;
   }
   if(processVersionId) {
     processVersionIdArr.value = processVersionId;
   }
};
...
const submitForm = async () => {
  ...
  formLoading.value = true;
  try {
    const data = formData.value as unknown as ProcessDetailTaskVO;
    if (formType.value === 'add') {
      if (partVersionIdArr.value && procedureIdArr.value && processVersionIdArr.value) {
        data.partVersionIdArr = partVersionIdArr.value;
        data.procedureIdArr = procedureIdArr.value;
        data.processVersionIdArr = processVersionIdArr.value;
        data.status = 1;
        await ProcessDetailTaskApi.assignProcessTask(data);
      }
      message.success(t('common.createSuccess'));
    } else {
      await  ProcessDetailTaskApi.updateProcessTask(data);
