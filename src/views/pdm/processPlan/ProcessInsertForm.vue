  ...
  <Dialog :title="dialogTitle" v-model="dialogVisible" width="900px">
    <el-form ref="formRefPlan" :model="formProcessPlan" :rules="formProcessRules" label-width="110px">
      ...
      <el-row>
        <el-col :span="12">
          <el-form-item label="关重属性" prop="property">
            <el-select v-model="formProcessPlan.property" clearable class="!w-400px">
              <el-option v-for="dict in getIntDictOptions(DICT_TYPE.PDM_PROCESSPLAN_PROPERTY) " :key="dict.value"
        ...
        <el-col :span="12">
          <el-form-item label="单机数量" prop=" singleQuatity">
            <el-input v-model="formProcessPlan.singleQuatity" pleaceholder="请输入单机数量" />
          </el-form-item>
        </el-col>
      </el-row>
      ...
      <el-row>
        ...
        <el-col :span="12">
          <el-form-item label="材料类码" prop="materialCode" label-width="120px">
...
const formProcessRules = reactive({
  ...
  materialNumber: [{ required: true, message: '请选择材料牌号', trigger: 'blur' }],
})
...
/** 打开弹窗 */
const open = async (type: string, id?: number, partVersionId: string, processCode: string, projectCode: string, projPartBomId: string) => {
  //对话框显示
  ...
  queryParamsProcess.projPartBomId = projPartBomId
  resetProcessForm()
  await generateData()
  if (id) {
...
const submitProcessForm = async () => {
  ...
  try {
    ...
    dataProcess.projPartBomId = queryParamsProcess.projPartBomId
    dataProcess.materialDesg = dataProcess.materialCode
    if (dataProcess.id) {
