  ...
  <ContentWrap>
    ...
    >
      ...
      <el-table-column label="检测工具 " align="center" prop="inspectionTool">
        <template #default="scope">
          <dict-tag :type="DICT_TYPE.QMS_INSPECTION_TOOL" :value="scope.row.inspectionTool"/>
        </template>
      </el-table-column>
...
<script setup lang="ts">
  ...
  import {FeasibilityDetailApi,ProcedureSchemaItemVO} from "@/api/pdm/feasibilityDetail";
  ...
  /** 检测项目 列表 */
  defineOptions({name: 'ProcedureInspectionItemChoose'})
  ...
  const list = ref < ProcedureSchemaItemVO[] > ([]) // 列表的数据
  ...
  watch(
    ...
    async (val) => {
      ...
      try {
        ...
        formData.value = await FeasibilityDetailApi.getProcedureSchemaItem(val)
      } finally {
   ...
   * 实现单选行
   */
  const selectionInspectionItem = ref < ProcedureSchemaItemVO[] > ()
