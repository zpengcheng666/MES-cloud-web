<template>
    ...
    <el-col :span="21" :xs="24">
      <el-row :gutter="10">
        <el-col :span="16" :xs="24">
            ...
            >
              ...
              <el-form-item label="状态" prop="status">
                <el-select v-model="queryParams.status" placeholder="请选择状态" clearable class="!w-180px">
                  <el-option
                    v-for="dict in getIntDictOptions(DICT_TYPE.PDM_FEASIBILITY_TASK_STATUS)"
                    :key="dict.value"
                    :label="dict.label"
                    :value="dict.value"
                  />
                ...
                <el-button
                  v-hasPermi="['pdm:feasibilityTask:add']"
                  plain
                  type="primary"
                  @click="submitApproval(
              selectionList.map((item) => item.id),
              selectionList.map((item) => item.partVersionId)
            )"
          ...
          <!-- 第二个ContentWrap，包含评估任务目录 -->
          <ContentWrap>
            <el-collapse v-model="activePartName">
              <el-collapse-item title="评估任务目录" name="1">
                <el-table ref="multipleTable" :data="partDataList" style="cursor: pointer;height:220px;overflow-y: auto" @row-click="partChange"
                  ...
                  <el-table-column align="center" label="零件图号" prop="partNumber" width="130px"/>
                  <el-table-column align="center" label="零件名称" prop="partName" />
                  <el-table-column align="center" label="版次" prop="partVersion" width="55px"/>
                  <el-table-column align="center" label="状态"  prop="status" width="80px">
                    <template #default="scope" >
                      <dict-tag :type="DICT_TYPE.PDM_FEASIBILITY_TASK_STATUS " :value="scope.row.status" />
                    </template>
                  </el-table-column>
                  <el-table-column align="center" label="负责人" prop="reviewer"  width="115px" />
                  <el-table-column :formatter="dateFormatter" align="center" label="评估截止时间" prop="deadline"  width="110px" />
                  <el-table-column label="操作" align="center"  width="60px" >
                    <template #default="scope">
                      <el-button v-if="scope.row.status === '1'" v-hasPermi="['pdm:feasibilityTask:update']" link type="primary" @click="openForm('update',scope.row.taskId)">
                        编辑
                      </el-button>
                    </template>
                  </el-table-column>
  ...
  <FeasibilityTaskForm ref="formRef" @success="getPartData" />
</template>
...
import FeasibilityTaskForm from "@/views/pdm/feasibilityTask/feasibilityTaskForm.vue";
...
defineOptions({name: 'feasibilityTask'});
...
const getStatusList=async ()=>{
  statusList.value = await FeasibilityTaskApi.getPartListByFeasibilityTaskId(statusList.status);
...
const getPartData = async () => {
  partDataList.value = await FeasibilityTaskApi.getPartListByFeasibilityTaskId(queryParams)
}
...
/** 指派/编辑操作 */
const formRef=ref()
const openForm = (type: string, taskId?: string, partVersionIds?: string[], projectCode?: string,status?: string) => {
  formRef.value.open(type,taskId, partVersionIds,projectCode,status)
}
const submitApproval = async (ids: number[], partVersionIds: string[]) => {

  const hasAssignedTasks = selectionList.value.some((item) => item.status === '1');
  if (hasAssignedTasks) {
    ElMessage.error('请选择未指派的评估任务')
    return;
  }

  try {
    selectionList.value = selectionList.value.filter((item) => !ids.includes(item.partVersionId));
    openForm('add', null, partVersionIds, queryParams.projectCode);
  } catch (error) {
...
/** 选中操作 */
const selectionList = ref<FeasibilityTaskVO[]>([])
const handleSelectionChange = (rows: FeasibilityTaskVO[]) => {
