<template>
  <Dialog v-model="dialogVisible" :title="dialogTitle">
    <el-form ref="formRef" v-loading="formLoading" :model="formData" :rules="formRules" label-width="140px">
      ...
      <el-form-item label="工艺准备完成时间" prop="deadline">
        <el-date-picker v-model="formData.processPreparationTime" type="date" disabled />
      </el-form-item>
    </el-form>
    <template #footer>
...
defineOptions({ name: 'ProcessTaskForm' });
...
const formData = ref({
  ...
  status: '',
  deadline: '',
  processPreparationTime: '',
});
const formRules = reactive({
  reviewer: [{ required: true, message: '负责人未选择', trigger: 'blur' }],
  deadline: [{ required: true, message: '编制截止时间不能为空', trigger: 'change' }]
});

const disabledDate = (time) => {
  const startDate = dayjs(time).isBefore(dayjs().startOf('day'), 'day');
  const endDate = dayjs(time).isAfter(dayjs(formData.value.processPreparationTime).startOf('day'),'day');
  return startDate || endDate;
};

const formRef = ref(); // 表单 Ref

const taskIdArr = ref([]) // 任务id数组
...
/** 打开弹窗 */
const open = async (type: string, taskId?: string,taskIds?: string[], processPreparationTime?: Date) => {
  ...
  // }
  if (taskIds) {
    taskIdArr.value = taskIds
    formData.value.processPreparationTime = processPreparationTime
  }
...
const submitForm = async () => {
  ...
  try {
    ...
    if (formType.value === 'add') {
      console.log(taskIdArr.value)
      if (taskIdArr.value) {
        data.taskIdArr = taskIdArr.value
        data.status = 1;
        await ProcessTaskApi.addProcessTaskNew(data);
      }
