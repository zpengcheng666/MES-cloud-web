    ...
    <el-col :span="18" :xs="24">
      <ContentWrap v-show="showProcess">
        <el-tabs model-value='1' class="demo-tabs">
          <el-tab-pane label="补加工工艺属性" name="1" style="height:600px;overflow-y: auto;overflow-x: hidden">
      ...
      <ContentWrap v-show="showProcedure">
        <el-tabs model-value='1' class="demo-tabs">
          ...
          <el-tab-pane label="工序草图" name="3" style="height:500px;overflow-y: auto">
      ...
      <ContentWrap v-show="showStep">
        <el-tabs model-value='1' class="demo-tabs">
          ...
          <el-tab-pane label="执行程序" name="4" style="height:500px;overflow-y: auto">
...
import {ProcessSupplementApi} from "@/api/pdm/processSupplement";

defineOptions({ name: 'PDMProcessSupplementDetail' })
...
const getTree = async () => {
  treeList.value = []
  queryParamsResource.processVersionId = props.id || queryId;
  const data = await ProcessSupplementApi.getProcessSupplementTreeList(queryParamsResource)
...
const handleNodeClick = async (row) => {
  if (row.type === 0) {//工艺规程节点
    ...
    processForm.value = await ProcessSupplementApi.getProcessSupplement(row.id)
