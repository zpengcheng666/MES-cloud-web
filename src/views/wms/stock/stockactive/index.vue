      ...
      <el-table-column label="操作" align="center" width="200px">
        <template #default="scope">
          <el-button
            link
            type="primary"
            @click="openForm('off', scope.row.id)"
            v-hasPermi="['wms:stock-off-shelf:active']"
          >
            下架
          </el-button>
          <el-button
...
<!--          <el-button
  ...
  <!-- 表单弹窗：下架 -->
  <StockActiveForm ref="formRef" @success="getList" />
</template>
...
import StockActiveForm from './StockActiveForm.vue'
import { StockActiveApi } from '@/api/wms/stockactive';


/** 物料库存 列表 */
defineOptions({ name: 'StockActive' })
...
const getList = async () => {
  loading.value = true
  try {
    const data = await StockActiveApi.getStockOffShelfPage(queryParams)
...
/** 上架按钮操作 */
const handleOnShelf = async (id: string) => {
  try {
    // 提交的二次确认
    await message.confirm("确认上架此物料？")
    // 发起删除
    await MaterialStockApi.stockOnShelf(id)
