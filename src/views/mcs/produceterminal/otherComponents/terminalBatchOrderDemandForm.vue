    ...
    <el-form ref="formRef" :model="formData" :rules="formRules" label-width="100px" v-loading="formLoading">
      <!-- <el-form-item label="订单编号" prop="orderId">
        <el-select v-model="formData.orderId" @change="getBatchList" filterable placeholder="请选择订单编号">
          <el-option v-for="order in orderList" :key="order.id" :label="order.orderNumber" :value="order.id" />
        </el-select>
      </el-form-item>
      <el-form-item label="批次编码" prop="batchId">
        <el-select v-model="formData.batchId" @change="getBatchRecordList" filterable placeholder="请选择订单编号">
          <el-option v-for="batch in batchList" :key="batch.id" :label="batch.batchNumber" :value="batch.id" />
        </el-select>
      </el-form-item> -->
      <el-form-item label="任务编码" prop="batchRecordId">
        <el-select v-model="formData.batchRecordId" @change="batchRecordSelect" filterable placeholder="请选择订单编号">
          <el-option v-for="batch in batchRecordList" :key="batch.id" :label="batch.number" :value="batch.id" />
        </el-select>
      </el-form-item>
      ...
      <el-form-item label="设备" prop="deviceId">
        <el-select v-model="formData.deviceId" filterable placeholder="请选择生成单元" disabled>
      ...
      <el-form-item label="资源类型" prop="resourceType">
        <el-select v-model="formData.resourceType" @change="clearTypeId" placeholder="请选择资源类型">
          <el-option v-for="dict in materialTypeList" :key="dict.id" :label="dict.name" :value="dict.id" />
        </el-select>
      </el-form-item>
...
import ResourceTypeSelectView from '../../batchorderdemand/components/ResourceTypeSelectView.vue';
...
/** 批次订单需求 表单 */
defineOptions({ name: 'terminalBatchOrderDemandForm' });
...
/** 打开弹窗 */
const open = async (stationData) => {
  dialogVisible.value = true;
  dialogTitle.value = '新增';
  resetForm();

  batchRecordList.value = await TerminalApi.getBatchPlanByWorkstation(stationData.id);
  unitList.value = await BatchOrderDemandApi.getUnitList();
  formData.value.processingUnitId = stationData.lintStationGroup || stationData.equipmentStationType;
  await getDeviceList(formData.value.processingUnitId);
