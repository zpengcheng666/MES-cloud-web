      ...
      <el-table v-loading="loading" :data="list" :stripe="true" :show-overflow-tooltip="true" >
        ...
        <el-table-column type="expand" align="center">
          <template #default="scope">
            <el-tabs model-value="OfflineErrorDetail">
              <el-tab-pane label="错误数据明细" name="OfflineErrorDetail">
                <OfflineErrorDetail :items-value="scope.row.items"/>
              </el-tab-pane>
            </el-tabs>
          </template>
        </el-table-column>
...
import OfflineErrorDetail from "@/views/dc/deviceoffline/detail/OfflineErrorDetail.vue";
import {OfflineErrorApi, OfflineErrorVO} from "@/api/dc/deviceofflineerror";

/** 设备 表单 */
defineOptions({ name: 'OfflineErrorForm' })
...
const dialogTitle = ref('错误日志') // 弹窗的标题
const list = ref < OfflineErrorVO[] > ([]) // 列表的数据
...
const getList = async () => {
  loading.value = true
  try {
    const data = await OfflineErrorApi.getOfflineErrorPage(queryParams)
    list.value = data.list
    total.value = await OfflineErrorApi.queryOfflineErrorCount(queryParams)
  } finally {
    loading.value = false
  }
}
...
/** 子表的表单 */
const subTabsName = ref('deviceErrorDetail')
