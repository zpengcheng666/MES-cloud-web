...
<!--        </el-table-column>-->
        ...
        <el-table-column label= "异常时间" align="center" prop="time" :formatter="dateFormatter"/>
      </el-table>
...
const dialogTitle = ref('在线异常日志') // 弹窗的标题
...
const getList = async () => {
  loading.value = true
  try {
    const data = await OfflineAlarmApi.queryOlineStatusDetailPage(queryParams)
    list.value = data.list
    total.value = await OfflineAlarmApi.queryOlineStatusDetailCount(queryParams)
  } finally {
    loading.value = false
  }
}
