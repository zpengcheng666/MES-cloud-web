...
<script setup lang="ts">
    ...
    /** 物料类型 表单,来自MCC,现用来处理没有的物料类型(工件也是物料) */
    defineOptions({name: 'HandleMaterialConfigForm'})
    ...
    /** 打开弹窗 */
    const open = async (marerialNumber,id) =>
    {
      orderId.value = id;
        console.log(marerialNumber);
        dialogVisible.value = true
        dialogTitle.value = t('action.' + 'create')
        resetForm()
        // formData.value.materialNumber = marerialNumber;
        getMaterialType()
    ...
    const cancel = async () => {
        ...
        if (formData.value.materialNumber) {
            const data = formData.value as unknown as MaterialConfigVO
