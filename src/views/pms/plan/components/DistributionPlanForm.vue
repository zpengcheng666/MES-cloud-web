<template>
  <Dialog title="生产加工" v-model="dialogVisible" width="600" @close="closeDialog">
    <el-form
      ref="formRef"
      :model="formData"
      :rules="formRules"
...
const open =async (row)=>{
  ...
  try {
    ...
    // const order =await OrderApi.getOrderByProjectId(row.projectId);
    // if(order.outsource){
    //   formData.value.outsourceNmae = '是'
    // }else {
    //   formData.value.outsourceNmae = '否'
    // }
    // formData.value.projectCode = row.projectCode;
    // formData.value.partNumber = row.partNumber;
    // formData.value.materialNumber = row.materialNumber;
...
const getRelationRows = (rows)=>{
  ...
  if(rows.length>0){
    formData.value.processStep = rows[0].step
  }
