    ...
    <el-collapse v-model="showDeviceInfo">
      <el-collapse-item title="设备" name="1">
        <el-table :data="deviceData" border>
          ...
          <el-table-column prop="wastage" label="损耗" align="center"/>
...
const getTechnologyAssessmentInfo = async ()=>{
  ...
  data.deviceList.forEach((item)=>{
    ...
    if(item.amount===null||item.amount===undefined){
      item.amount = devicedata[index]?.amount!=null?devicedata[index]?.amount:0;
    }
    if(item.purchaseAmount===null||item.purchaseAmount===undefined){
      item.purchaseAmount = devicedata[index]?.purchaseAmount!=null?devicedata[index]?.purchaseAmount:0;
    }
    ...
    if(item.predictPrice===null||item.predictPrice===undefined){
      item.predictPrice = devicedata[index]?.predictPrice!=null?devicedata[index]?.predictPrice:0;
    }
    if(item.wastage===null||item.wastage===undefined){
      item.wastage = devicedata[index]?.wastage!=null?devicedata[index]?.wastage:0;
    }
