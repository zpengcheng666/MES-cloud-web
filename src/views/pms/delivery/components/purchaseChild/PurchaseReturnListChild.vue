...
import { ConsignmentReturnApi } from '@/api/ppm/consignmentreturn/index'
...
const getList = async () => {
  loading.value = true
  try {
    // list.value = await PurchaseConsignmentApi.getPurchaseConsignmentDetailListByConsignmentId(props.returnId);
    //list.value = await ConsignmentReturnApi.getConsignmentReturnDetailListByConsignmentReturnId(props.returnId)
    const data = await DeliveryApi.getShippingDetailByPurchaseId(props.returnId)
