<template>
  <ContentWrap>
    ...
    <el-form class="-mb-15px" :model="queryParams" ref="queryFormRef" :inline="true" label-width="96px">
      <el-form-item label="配件品牌" prop="toolBrandId">
        <el-select v-model="queryParams.toolBrandId" placeholder="请选择配件品牌" clearable class="!w-240px">
          <el-option v-for="dict in getIntDictOptions(DICT_TYPE.KNIFETOOL_BRAND)" :key="dict.value" :label="dict.label"
            :value="dict.value" />
        </el-select>
      </el-form-item>
      <el-form-item label="配件分类名称" prop="toolCategoryId">
        <el-tree-select placeholder="请选择配件分类" check-strictly :props="defaultProps" v-model="queryParams.toolCategoryId"
          :data="treeSelect" :render-after-expand="false" @change="handleChange" class="!w-240px" />
      </el-form-item>
      <el-form-item label="配件名称" prop="name">
        <el-input v-model="queryParams.name" placeholder="请输入配件名称" clearable @keyup.enter="handleQuery"
          class="!w-240px" />
      </el-form-item>
      ...
      <el-form-item label="配件类码" prop="code">
        <el-input v-model="queryParams.code" placeholder="请选择配件类码" clearable @keyup.enter="handleQuery"
          class="!w-240px" />
      </el-form-item>
      ...
      <el-form-item>
        ...
        <el-button type="primary" plain @click="openPage('create', null)" v-hasPermi="['knifetool:information:create']">
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list" :stripe="true" :show-overflow-tooltip="true">
      <el-table-column label="配件分类" align="center" prop="toolCategoryValue" />
      <el-table-column label="配件类码" align="center" prop="code" />
      <el-table-column label="配件名称" align="center" prop="name" />
      <el-table-column label="配件品牌" align="center" prop="toolBrandId">
        <template #default="scope">
          <dict-tag :type="DICT_TYPE.KNIFETOOL_BRAND" :value="scope.row.toolBrandId" />
        </template>
      </el-table-column>

      <el-table-column label="配件通用规格" align="center" prop="spec" />
      <!-- <el-table-column label="配件类型" align="center" prop="toolTypeId">
        <template #default="scope">
...
const openPage = (type: string, id: any) => {
  if (null !== id) {
    push(`/partsInfoData/${type}/${id}`)
  } else {
    push(`/partsInfoData/${type}`)
  }
}
...
const handleExport = async () => {
  try {
    ...
    download.excel(data, '配件基本信息.xls')
  } catch {
  } finally {
...
const handleStateChange = async (row) => {
  try {
    ...
    await message.confirm('确认要"' + text + '""' + row.name + '"配件基本信息吗?')
    // 发起修改状态
