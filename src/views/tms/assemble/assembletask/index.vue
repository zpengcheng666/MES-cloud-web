<template>
  <ContentWrap>
    ...
    >
      ...
      <el-form-item label="状态" prop="status">
        ...
        >
          <el-option
            v-for="dict in getIntDictOptions(DICT_TYPE.ASSEMBLE_TASK_STATUS)"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
        ...
        <template #default="scope">
          <el-tabs model-value="assembleRecord">
            <el-tab-pane label="刀具装配记录" name="assembleRecord">
              <AssembleRecordList :assemble-task-id="scope.row.id" />
            </el-tab-pane>
          </el-tabs>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="150" align="center">
        <template #default="scope">
          ...
          <el-button
            ...
            v-hasPermi="['tms:assemble-task:delete']"
          >
            删除
          </el-button>
        </template>
  ...
  <!-- 表单弹窗：添加/修改 -->
  <AssembleTaskForm ref="formRef" @success="getList" />
</template>
...
import AssembleTaskForm from './AssembleTaskForm.vue'
...
const handleAccept = async (row: AssembleTaskVO) => {
  ...
  ElMessageBox.confirm('是否确认接单？', '提示', {
    ...
    row.status= 2;
    row.operator = userId;
    await AssembleTaskApi.updateAssembleTask(row)
