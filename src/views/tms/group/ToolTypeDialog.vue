...
const open = async (data: any, ids: any) => {
  dialogTitle.value = [data === 1 ? '刀柄' : data === 2 ? '刀具' : '配件'] + '基本信息'
  queryParams.value.queryType = data
  queryParams.value.ids = ids.join(",")
...
const getList = async () => {
  loading.value = true
  try {
    const data = await ToolConfigApi.getToolConfigPageByType(queryParams.value)
