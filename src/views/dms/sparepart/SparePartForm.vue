  ...
  <Dialog :title="dialogTitle" v-model="dialogVisible">
    ...
    >
      ...
      <el-form-item label="备件类型" prop="type">
        <el-input v-model="formData.type" placeholder="请输入备件类型" />
      </el-form-item>
...
/** 备品/备件 表单 */
defineOptions({ name: 'SparePartForm' })
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await SparePartApi.getSparePart(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as SparePartVO
