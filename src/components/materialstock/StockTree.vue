<template>
  ...
  <div class="head-container">
    <el-tree ref="treeRef" :data="stockList" :expand-on-click-node="false" :filter-node-method="filterNode"
...
defineOptions({ name: 'StockTree' })

const wName = ref('')
const stockList = ref<Tree[]>([]) // 树形结构
...
/** 获得部门树 */
const getTree = async () => {
  const res = await MaterialStockApi.stockMenuTree(type)
  stockList.value = res
}
